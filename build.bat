...
rem Version is single-sourced from package.json — passed to ISCC via /D.
set "VC="
for /f "tokens=2 delims=:, " %%a in ('findstr /c:""version"" package.json') do set "VC=%%~a"
if "%VC%"=="" (
...
"%ISCC%" /DMyAppVersion=%VC% installer\llama-center.iss || goto :err

:done
echo.
echo Done:
echo   App:      dist\llama-center\llama-center.exe
echo   Installer: dist\llama-center-setup-%VC%.exe
goto :eof
