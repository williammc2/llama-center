...
| P5 settings/tray/autostart | ✅ DONE | Settings page (shell); tray icon (pystray, runtime-generated icon, menu Show/Start/Stop/Check updates/Quit); close-to-tray; `--minimized` login start; autostart (HKCU Run / XDG .desktop, applied on save_config); autoStartLlamaSwap (port must be free) |
